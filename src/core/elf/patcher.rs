...
pub fn patch_for_zl(
    ...
    if let Some(ref orig_interp) = info.interpreter {
        if let Some(new_interp) = mapping.remap_interpreter(orig_interp) {
            ...
            patcher.set_interpreter(&c_interp).map_err(|e| {
                crate::error::ZlError::ElfPatch {
                    path: path.to_path_buf(),
                    message: e.to_string(),
                }
    ...
    if let Some(runpath) = mapping.compute_runpath(path, &info.needed_libs) {
        let c_runpath = std::ffi::CString::new(runpath).map_err(|e| {
            crate::error::ZlError::ElfPatch {
                path: path.to_path_buf(),
                message: e.to_string(),
            }
    ...
    patcher.finish().map_err(|e| crate::error::ZlError::ElfPatch {
        path: path.to_path_buf(),
        message: e.to_string(),
    })?;
...
pub fn set_runpath(path: &Path, new_runpath: &str) -> ZlResult<()> {
    ...
    let c_runpath = std::ffi::CString::new(new_runpath).map_err(|e| {
        crate::error::ZlError::ElfPatch {
            path: path.to_path_buf(),
            message: e.to_string(),
        }
